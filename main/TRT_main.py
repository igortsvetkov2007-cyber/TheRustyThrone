...
async def successful_payment(message):
    if message.chat.id not in users_buy_dict.keys():
        ...
        return
    cursor.execute('update Users set ?=?+? where chat_id=?',(users_buy_dict[message.chat.id][0],users_buy_dict[message.chat.id][0],users_buy_dict[message.chat.id][1],message.chat.id))
    connection.commit()
